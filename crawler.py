...
from Feeling import get_emotion
import re as regex
...
def build_graph(nodes):
    ...
    g=nx.barabasi_albert_graph(len(nodes),len(nodes)//3)
    PageRank=nx.pagerank(g,0.32)
    print (PageRank)
...
def analyse():
    ...
    series = target(sent)
    # new_emotion,emotion_library=get_emotion(sent)
    count=0
    ...
    for i in series :
        ...
        if(count == n+1):
            break
    build_graph(["anger","disgust","surprise","anticipation","trust","fear","sad"])
analyse()